...
describe('TimeIndicator', () => {
  ...
  it('renders time information with initialState', async () => {
    render(<TimeIndicator initialState={mockTimeState} />);
    // Wait for async fetchWarningThreshold to complete
    await waitFor(() => {
      const container = screen.getByText('⏰').closest('div');
      expect(within(container!).getAllByText(/30:00/).length).toBeGreaterThan(0);
      expect(within(container!).getAllByText(/60:00/).length).toBeGreaterThan(0);
  ...
  it('shows orange color when time is low', async () => {
  ...
  it('shows green color when time is sufficient', async () => {
    render(<TimeIndicator initialState={mockTimeState} />);
    // Wait for async fetchWarningThreshold to complete
    await waitFor(() => {
      const container = screen.getByText('⏰').closest('div');
  ...
  it('shows limit reached message when isLimitReached is true', async () => {
