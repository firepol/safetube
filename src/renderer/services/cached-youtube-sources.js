...
exports.CachedYouTubeSources = void 0;
...
class CachedYouTubeSources {
    static async loadSourceVideos(source) {
        ...
        if (source.type === 'youtube_channel') {
            ...
            const allVideos = await youtube_1.YouTubeAPI.getChannelVideos(channelId, 50); // TODO: handle pagination
            ...
            const allVideos = await youtube_1.YouTubeAPI.getPlaylistVideos(playlistId, 50); // TODO: handle pagination
            newVideos = await fetchNewYouTubeVideos(allVideos, cache?.videos || []);
        }
...
exports.CachedYouTubeSources = CachedYouTubeSources;
// Helpers
...
async function fetchNewYouTubeVideos(allVideoIds, cachedVideos) {
    // Fetch details for videos not in cache
    const cachedIds = new Set(cachedVideos.map(v => v.id));
    const newIds = allVideoIds.filter(id => !cachedIds.has(id));
    const details = await Promise.all(newIds.map(id => youtube_1.YouTubeAPI.getVideoDetails(id)));
    // Map to cache format
