...
export class CachedYouTubeAPI {
  static async getVideoDetails(videoId: string): Promise<any> {
    ...
    if (cached) {
      logVerboseRenderer(`[CACHE] Using cached video details for ${videoId}`);
      return cached;
    }

    // Fetch from API and cache
    logVerboseRenderer(`[CACHE] Fetching video details for ${videoId}`);
  ...
  static async getVideoStreams(videoId: string): Promise<{ videoStreams: any[]; audioTracks: any[] }> {
    ...
    if (cached) {
      logVerboseRenderer(`[CACHE] Using cached video streams for ${videoId}`);
      return cached;
    }

    // Fetch from API and cache
    logVerboseRenderer(`[CACHE] Fetching video streams for ${videoId}`);
