...
export function registerFavoritesHandlers() {
  ...
  // Populate channelId for YouTube channel sources
  ipcMain.handle('sources:populate-channel-ids', async () => {
    try {
      const sourcesPath = AppPaths.getConfigPath('videoSources.json');

      if (!fs.existsSync(sourcesPath)) {
        return { success: false, error: 'Video sources file not found' };
      }

      // Get main settings for YouTube API key
      ...
      for (const source of sources) {
        if (source.type === 'youtube_channel' && !source.channelId) {
          try {
            let channelId: string | null = null;

            // Try to extract channelId directly from URL first
            ...
            if (!channelId) {
              ...
              if (match) {
                ...
                logVerbose(`[IPC] Resolving channelId for @${username}`);

                try {
                  const searchResult = await youtubeApi.searchChannelByUsername(username);
                  channelId = searchResult.id.channelId;
                } catch (error) {
                  logVerbose(`[IPC] Search failed, trying channel details for: ${username}`);
                  // Fallback: sometimes the @ URL redirects, try getting channel details
            ...
            if (channelId) {
              ...
              updatedCount++;
              logVerbose(`[IPC] Added channelId ${channelId} to source ${source.title}`);
            } else {
              const errorMsg = `Unable to resolve channelId for ${source.title} (${source.url})`;
              errors.push(errorMsg);
              logVerbose(`[IPC] ${errorMsg}`);
            }
          } catch (error) {
            const errorMsg = `Error resolving channelId for ${source.title}: ${error instanceof Error ? error.message : String(error)}`;
            errors.push(errorMsg);
            logVerbose(`[IPC] ${errorMsg}`);
          }
        }
      ...
      logVerbose(`[IPC] Populated channelId for ${updatedCount} sources`);
      ...
      log.error('[IPC] Error populating channel IDs:', error);
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' };
    }
  });
}
