...
const TEST_DB_PATH = '/tmp/claude/test-schema-database.db';
...
describe('SchemaManager', () => {
  ...
  test('should initialize Phase 1 schema successfully', async () => {
    await schemaManager.initializeSchema('phase1');
  ...
  test('should validate Phase 1 schema correctly', async () => {
    ...
    const validation = await schemaManager.validateSchema('phase1');
    expect(validation.isValid).toBe(true);
    expect(validation.errors).toHaveLength(0);
    expect(validation.missingTables).toHaveLength(0);
    expect(validation.phase).toBe('phase1');
  });
